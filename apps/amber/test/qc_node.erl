...
handle_call({qc_set_receiver, Receiver}, _From, State)
  when
  is_pid(Receiver) -> {reply, ok, State#state{qc_receiver = Receiver}}.

handle_cast({received, #routing_msg{} = FullMsg}, #state{qc_receiver = QC_Rec, mid = MID} = State) ->
  try
    QC_Rec ! {ok, {?MODULE, MID}, FullMsg}
  catch
    error:badarg -> ok_anyway
  end,
  {noreply, State};
