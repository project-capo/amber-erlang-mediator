...
start_node({RegName} = MID, Conf) ->
  gen_server:start_link({local, RegName}, driver, {MID, Conf}, []).

receive_msg({RegName}, FullMsg) ->
  gen_server:cast(RegName, {received, FullMsg}).
