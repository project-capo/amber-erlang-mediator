...
init([]) ->
  ...
  Nodes = lists:filter(fun(X) ->
    is_record(X, supervised_node)
  end, AllNodes),

  NodesWR = [#supervised_node{mod_id = {router, {router}}, conf = []} | Nodes],

  Drivers = lists:filter(fun(X) ->
    is_record(X, supervised_driver)
  end, AllNodes),
  ...
  QN = fun({#supervised_node{mod_id = ModID, conf = Conf}, GID, DevTI}) ->
    mnesia:transaction(fun() ->
      mnesia:write(#routing_node{id = GID, mod_id = ModID, dev_ti = DevTI, conf = Conf})
    end)
  end,
  ...
  lists:foreach(QN, lists:zipwith(fun(A, B) ->
    {A, B, make_ref()}
  end, NodesWR, lists:seq(0, NodesWRN - 1))),
