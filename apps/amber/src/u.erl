...
tpl_vis(Mods)
  when
  is_list(Mods) -> [tpl_vis(M) || M <- Mods];

tpl_vis(Mod)
  when
  is_atom(Mod) -> [dbg:tpl(Mod, F, '_', [{'_', [], [{return_trace}]}]) || {F, _Ar} <- Mod:module_info(functions)].
...
r(Mods)
  when
  is_list(Mods) -> [r(M) || M <- Mods];

r(Mod)
  when
  is_atom(Mod) -> {module, _} = code:load_file(Mod),
  code:purge(Mod).
